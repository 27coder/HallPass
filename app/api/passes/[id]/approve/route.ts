...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Additional authorization: teachers can only approve passes from their students
  ...
  await prisma.pass.update({
    where: { id: id },
    data: { status: 'approved', approvedAt: new Date(), teacherId: session.user.id }
  })

  return NextResponse.json({ message: 'Pass approved' }, { status: 200 })
}